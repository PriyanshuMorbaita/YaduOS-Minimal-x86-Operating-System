...
static bool elf_valid_class(struct elf_header* elf_header){
    return elf_header->e_ident[EI_CLASS] == ELFCLASSNONE || elf_header->e_ident[EI_CLASS] == ELFCLASS32;
}

static bool elf_valid_encoding(struct elf_header* elf_header){
    return elf_header->e_ident[EI_DATA] == ELFDATANONE || elf_header->e_ident[EI_DATA] == ELFDATA2LSB;
}

static bool elf_is_executable(struct elf_header* elf_header){
    return elf_header->e_type == ET_EXEC && elf_header->e_entry >= PEACHOS_PROGRAM_VIRTUAL_ADDRESS;
}
...
static bool elf_has_section_header(struct elf_header* elf_header){
    return elf_header->e_shoff != 0;
}
...
char* elf_str_table(struct elf_header* elf_header){
...
int elf_validate_loaded(struct elf_header* header){
 return (elf_valid_signature(header) && elf_valid_class(header) && elf_valid_encoding(header) 
        && elf_has_program_header(header) && elf_is_executable(header)) 
