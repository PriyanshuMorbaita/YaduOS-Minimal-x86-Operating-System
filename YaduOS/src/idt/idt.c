...
static INTERRUPT_CALLBACK_FUNCTION interrupts_callbacks[PEACHOS_TOTAL_INTERRUPTS];
...
void interrupt_handler(int interrupt, struct interrupt_frame* frame){ 
    kernel_page();
    if(interrupts_callbacks[interrupt] != 0){
        task_current_save_state(frame);
        interrupts_callbacks[interrupt](frame);
    }
    task_page();
    outb(0x20,0x20);
}
...
int idt_register_interrupt_callback(int interrupt, INTERRUPT_CALLBACK_FUNCTION interrupt_callback){
    ...
    interrupts_callbacks[interrupt] = interrupt_callback;
    return 0;
}
